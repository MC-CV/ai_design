...
class HMM():
    ...
    def Init_Array(self):
        for state0 in self.STATES:
            ...
            for state1 in self.STATES:
                self.array_A[state0][state1] = 0.0 # {'B': {'B': 0.0, 'M': 0.0, 'E': 0.0, 'S': 0.0}, 'M': {'B': 0.0, 'M': 0.0, 'E': 0.0, 'S': 0.0}, 'E': {'B': 0.0, 'M': 0.0, 'E': 0.0, 'S': 0.0}, 'S': {'B': 0.0, 'M': 0.0, 'E': 0.0, 'S': 0.0}}
        ...
        for key0 in self.array_A:
            for key1 in self.array_A[key0]:
                if self.array_A[key0][key1] == 0.0:
                    self.array_A[key0][key1] = -3.14e+100
                else:
                    self.array_A[key0][key1] = math.log(self.array_A[key0][key1] / self.count_dic[key0])
        # print(array_A)
        for key in self.array_B:
            for word in self.array_B[key]:
                if self.array_B[key][word] == 0.0:
                    self.array_B[key][word] = -3.14e+100
                else:
                    self.array_B[key][word] = math.log(self.array_B[key][word] /self.count_dic[key])
    ...
    def dist_tag(self):
        ...
        self.array_E['M']['begin'] = -3.14e+100
        self.array_E['E']['begin'] = -3.14e+100
        self.array_E['S']['begin'] = -3.14e+100
        self.array_E['B']['end'] = -3.14e+100
        self.array_E['M']['end'] = -3.14e+100
        self.array_E['E']['end'] = 0
        self.array_E['S']['end'] = -3.14e+100

    #将字典转换成数组
    def dist_word(self,word0,word1,word2,array_b):
        if self.dist_tag(word0,array_b) == 'S':
            self.array_E['B'][word1] = 0
            self.array_E['M'][word1] = -3.14e+100
            self.array_E['E'][word1] = -3.14e+100
            self.array_E['S'][word1] = -3.14e+100
        return
    ...
    def Viterbi(self,sentence,array_pi,array_a,array_b):
        ...
        prob, state = max([(tab[len(sentence) - 1][state], state) for state in self.STATES])
        return path[state]
