...
import os
import json
from HMMseg.utils import analyse
from HMMseg.model import HMM
from HMMseg.dataloader import dataprocessing
import argparse


root = r'D:\Users\53263\Courses\Artificial intelligence programming\final\HMMwordseg\icwb2-data'
root_out = r'D:\Users\53263\Courses\Artificial intelligence programming\final\HMMwordseg'
output_path = r'D:\Users\53263\Courses\Artificial intelligence programming\final\HMMwordseg\outputs'
def parse_args():
    parser = argparse.ArgumentParser(description='train & test HMM model')
    parser.add_argument('-d','--dataset',default='pku',help='the type of datasets that you chosed(as,cityu,msr,pku)')
    parser.add_argument('-m','--mode',default='train',help='train or eval or just inference')
    parser.add_argument('-s','--res',action='store_true',help='save the weights')
    parser.add_argument('-o','--out',default=output_path,help='where to output the results')
...
def train(model,trainset,tp):
    for line in tqdm(trainset):
            ...
            model.line_num += 1
            word_list = [] # ['上', '海', '浦', '东', '开', '发', '与', '法', '制', '建', '设', '同', '步']
            for k in range(len(line)):
                if line[k] == ' ' or line[k] == '\u3000':continue
            ...
            line_state = []     #这句话的状态序列 ['B', 'E', 'B', 'E', 'B', 'E', 'S', 'B', 'E', 'B', 'E', 'B', 'E']
            for i in line:
                if i == '':
                    continue
            ...
            for p in range(len(line_state)):
                ...
                model.array_B[line_state[p]][word_list[p]] += 1  # array_B用于计算发射概率 {'B': {'上': 1.0, '海': 0.0, '浦': 1.0, '东': 0.0, '开': 1.0, '发': 0.0, '与': 0.0, '法': 1.0, '制': 0.0, ...}, 
...
def inference(model,testset,tp,dir):
    ...
    with open(os.path.join(root_out,'results',results_name),'r',encoding='utf-8') as f:
        array_A = json.load(f)
    ...
    for line in tqdm(testset):
        ...
        if line == '':
            output = output + '\n'
            continue     
        tag = model.Viterbi(line, array_Pi, array_A, array_B) # ['B', 'E', 'B', 'E', 'B', 'E', 'B', 'E', 'B', 'E', 'B', 'E', 'B', 'E', ...]
        seg = analyse.tag_seg(line, tag) # ['外商', '投资', '企业', '成为', '中国', '外贸', '重要', '增长', '点']  
        temp_list = '' # 存储
...
def main():
    ...
    model = HMM()
    type_data = args.dataset
    trainset,testset,goldset = dataprocessing(type_data,root)
    if args.mode == 'train':
        model.Init_Array()
    ...
    if args.res or args.mode == 'train':
        ...
        with open(os.path.join(root_out,'results',results_name),'w',encoding='utf-8') as f:
